...
- 완료 보고에는 반드시 `검증 중 새로 발견한 문제`, `그 문제를 위해 추가로 수정한 내용`, `그 수정 뒤 다시 돌린 검증`, `AGENTS 확인 기록`이 포함돼야 한다.
- `AGENTS 확인 기록`은 최소 `총 재독 횟수`와 `시작 게이트 / 실행 직전 / 완료 직전` 같은 재독 시점을 포함한다.
...
- review 화면에는 현재 분류 현황과 `신청 메일 / 사람 검토 필요 / 비신청 메일` 기준 설명, 사용자 분류 기준 메모 안내를 함께 보여준다.
- 유효한 OpenAI API key가 없으면 review 화면과 설정 화면에서 `자동 요약과 분류가 제한됨`을 명시해, `요약 메모`와 신청 분류가 제한될 수 있음을 설명한다.
