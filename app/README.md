...
- 세이브 파일과 템플릿 경로는 Windows exe에서 `찾아보기` 버튼으로 고를 수 있고, 버튼은 exe 기준 기본 활성 상태에서 바로 시도한 뒤 실패 시 짧은 이유를 안내한다.
- `세이브 파일 도움말`은 별도 페이지 대신 홈 모달로 먼저 보여준다.
...
- pywebview launcher가 있고, 전용 창 연결이 늦더라도 바로 `브라우저 fallback`으로 단정하지 않고 재시도 후 안내하는 흐름을 쓴다.
